export const design = [
  {
    id: 1,
    header: 'Figma: 3+ years',
    content:
      'Figma is the main software I use when prototyping and designing websites.',
    certificates: 'Self Taught, and used extensively in my professional work.',
  },
  {
    id: 2,
    header: 'Illustrator: 4+ years',
    content: 'I mainly use it to create vector graphics and logos.',
    certificates: 'College Certificate, Multiple Udemy Certificates, used professionally',
  },
  {
    id: 3,
    header: 'Photoshop: 3+ years',
    content: 'For image optimization, asset creation, and composition.',
    certificates: 'College Certificate, Udemy Certificate, used professionally',
  },
  {
    id: 4,
    header: 'After Effects: 1 years',
    content:
      'Used to create lottie animations. It was pretty easy to pick up because of the Adobe Animate courses I took in college.',
    certificates: 'Self Taught',
  },
  {
    id: 5,
    header: 'Animate: 1 years',
    content:
      'I previously used Animate for several college courses, but I much prefer After Effects after trying it out.',
    certificates: 'College Certificate',
  },
];

export const development = [
  {
    id: 1,
    header: 'HTML & CSS: 5+ years',
    content:
      'I have experience working with BEM class naming conventions, advanced CSS such as flex-box, grid, keyframes, SVG manipulation/animation, and more.',
    certificates:
      '2 years proffesionally, College Certificate, Multiple Udemy Certificates, Thousands of hours of Self Taught/Research',
  },
  {
    id: 2,
    header: 'JavaScript: 4+ years',
    content:
      'My experience with JavaScript includes: async/await and Promises, objects array methods, destructuring, spread, functions default params, ternary operations, arrow functions and their impact on ".this", classes, ".this" keyword, in-built fetch and axios library, AudioContext Api, canvas manipulation, and much more.',
    certificates:
      '2 years proffesionally, College Certificate, Multiple Udemy Certificates, Hundreds of hours of Self Taught/Research',
  },
  {
    id: 3,
    header: 'React: 4+ years',
    content:
      'I have been almost exclusively focused on React development for over a year now. I wanted to dive deep into a single library so that I could become familiar with more advanced patterns, instead of relearning the basics of multiple technologies. I have used class based React, where I gained a lot of understanding of ".this", constructors, inheritance, and lifecycles. I mainly work with functional components w/ hooks. I\'m familiar with the context api, and creating compound components.',
    certificates:
      'Used some proffesionally, College Certificate, Multiple Udemy Certificates, Hundreds of hours of Self Taught/Research',
  },
  {
    id: 4,
    header: 'Redux: 2+ years',
    content:
      "Most of the projects I have used Redux on didn't need Redux, and could have been accomplished with a context provider, but I used Redux regardless to learn. I'm familiar with the core of action dispatch, reducers, and the store. I know how to use middleware to take care of calculations and asynchronous code before it is returned by the reducer.",
    certificates:
      'Multiple Udemy Certificates, Hundreds of hours of Self Taught/Research',
  },
  {
    id: 5,
    header: 'SCSS/Tailwind: 4+ years',
    content:
      'SASS and Tailwind are the primary way I write CSS for most projects.',
    certificates:
      '2 year proffesionally, Multiple Udemy Certificates, Hundreds of hours of Self Taught/Research',
  },
  {
    id: 6,
    header: 'framer-motion/lottie: 2+ years',
    content:
      "Framer is used when I feel like CSS transitions and keyframes can't get a job done, or when I need a more dynamic animation. If I need animations with a lot finer controls, or that are a lot more complex, I have started using lottie animations that I create in After Effects.",
    certificates: 'Self Taught',
  },
  {
    id: 7,
    header: 'Express: 2+ years',
    content:
      "I'm familiar with client/server architecture and CRUD operations. I have worked on projects involving Express and mongoose to communicate between the client and a backend database. These projects also included Redux and React.",
    certificates:
      'Multiple Udemy Certificates, Hundreds of hours of Self Taught/Research',
  },
  {
    id: 8,
    header: 'TypeScript: 2 years',
    content:
      "Most of my projects use TypeScript now.",
    certificates: 'Udemy Certificate, Self Taught',
  },
  {
    id: 9,
    header: 'NextJS. 2+ years.',
    content:
      "I try to use NextJs where I think it's appropriate. Sometimes it's a bit overkill.",
    certificates: 'Self Taught',
  },
  {
    id: 10,
    header: 'WordPress and PHP. 2+ years.',
    content:
      "",
    certificates: 'Used proffesionally, Self Taught',
  },
  {
    id: 11,
    header: 'Misc React/JS',
    content:
      'Zustand, React-query, Axios, Redux-Thunk, Formik, Font-Awesome Components, Jest, Bulma, Bootstrap, Material-UI, Tailwind, Storybook, Many external APIs, and dozens of smaller one off libraries.',
    certificates: 'Udemy Certificates, Self Taught',
  },
];

export const extras = [
  {
    id: 1,
    header: 'SQL',
    content:
      'The first database I started working with was mySql. I have a lot of experience with Access as well. I have used both Java, and PHP to connect to and manipulate sql table data, and I have a firm grasp on relational databases.',
    certificates: 'College Certificate, Professional Work Experience',
  },
  {
    id: 2,
    header: 'Java',
    content:
      'The first programming language I completed a college certificate in. Experience the same as PHP.',
    certificates: 'College Certificate',
  },
  {
    id: 3,
    header: 'Dreamweaver',
    content:
      "I took a Dreamweaver class in college. I prefer hand coding, so I haven't used it since, but I'm sure I could quickly relearn how to use it.",
    certificates: 'College Certificate',
  },
];
